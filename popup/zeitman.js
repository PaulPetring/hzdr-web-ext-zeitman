...
function provideLoginForm() {
  ...
  _result.html('<div style="max-height: 225px; overflow:hidden">' + $('#login').html() + '</div>');
...
function setValues() {
  var _iframe_hauptfenster = $("#iframe_hauptfenster").contents().find("body");
  _iframe_hauptfenster.find('select[name="' + tf_vonSS.attr("name") + '"]').val(tf_vonSS.val());
  _iframe_hauptfenster.find('select[name="' + tf_vonMM.attr("name") + '"]').val(tf_vonMM.val());
  _iframe_hauptfenster.find('select[name="' + tf_bisSS.attr("name") + '"]').val(tf_bisSS.val());
  _iframe_hauptfenster.find('select[name="' + tf_bisMM.attr("name") + '"]').val(tf_bisMM.val());
}
...
function showCalculatedWorktime() { // gives user feedback about current set work time
  ...
  if (tf_vonSS.length > 0) {
    ...
    if (hours >= 10 && mins >0) {
      ...
      tenHourdate.setTime(start_date.getTime() + (1000 * 60 * (10 * 60 + 15 + 30))); //ten hours plus breaks -4 avoiding rounding over 10h
      tenHourdate=round_date(tenHourdate);
      console.log("tenHourdate",tenHourdate)




      _alerts.html("You have selected more than ten hours of working time.It has been adjusted to the maximum of " + tenHourdate.getHours() + ":" + tenHourdate.getMinutes() + ".")

     //_alerts.find('#tenHourdateSet').click(function() {
          console.log("tenHourdate2",tenHourdate,tenHourdate.getMinutes(),tenHourdate.getHours())


          tf_bisMM.find("option").each(function() {console.log("removing for tf_bisMM"); $(this).removeAttr("selected"); });

          tf_bisMM.find('option[value="' + tenHourdate.getMinutes() + '"]').attr('selected', true).addClass("changed");
          tf_bisMM.val(tenHourdate.getMinutes());
          tf_bisMM.addClass("changed").attr("title", tenHourdate.getMinutes() + " ten hour mark -suggested by the plugin");


          tf_bisSS.find("option").each(function() { console.log("removing for tf_bisSS"); $(this).removeAttr("selected"); });
          console.log("tf_bisSS", tf_bisSS.find("option:selected"));
          tf_bisSS.find('option[value="' + tenHourdate.getHours() + '"]').attr('selected', true).addClass("changed");
          tf_bisSS.val(tenHourdate.getHours());
          tf_bisSS.addClass("changed").attr("title", tenHourdate.getHours() + " ten hour mark -suggested by the plugin");
          //setValues();
...
function preset_times() { //sets starting or end time to the current rounded date
  ...
  var rounded_date = round_date(cur_date)
...
function getStuff() {
  ...
  $("#iframe").contents().find("script").each(function() {
    ...
    if (alert_pos != -1) { //there is an alert
      ...
      _alerts.html(unescape(alert_text) + '<a href="#"> goto Zeitman </a>'); //give the alert to the user
      $('#alerts a').click(function() {
        browser.tabs.create({
          "url": "https://zeitweb.hzdr.de/scripts_zeitm/login.php"
        });
      })
  ...
  _result.find("td").last().append("<td></td>").html('<input id="save" type="submit" value="save">');
